...
import com.vodafone.v2xsdk4javav2.facade.configuration.SDKConfiguration;
...
public class V2XApplication {
    ...
    public void run() {
        try {
            ...
            int initRetries = 0;
            while (sdk.getV2XServiceState() != V2XServiceState.UPANDRUNNING) {
                Thread.sleep(1000L);
            ...
            long sequenceNumber = sdk.denmTrigger(DENMType.ACCIDENT, eventLocation);
            logger.info("  DENM triggered with sequence number: {}", sequenceNumber);
            logger.info("  Event Type: ACCIDENT");
